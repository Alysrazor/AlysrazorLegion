...
class spell_dk_death_grip_initial : public SpellScript
{
    ...
    void HandleOnCast()
    {
        ...
        if (!target->HasAuraType(SPELL_AURA_DEFLECT_SPELLS))
        {
            ...
            if (caster->IsPlayer() && caster->ToPlayer()->GetPrimarySpecialization() == TALENT_SPEC_DEATHKNIGHT_BLOOD)
...
public:
    ...
    class spell_dk_breath_of_sindragosa_AuraScript : public AuraScript
    {
        ...
        void OnTick(AuraEffect const* /*p_AurEff*/)
        {
            ...
            l_Caster->ModifyPower(POWER_RUNIC_POWER, l_Caster->GetPower(POWER_RUNIC_POWER) - 15);

            if (l_Caster->ToPlayer())
                l_Caster->ToPlayer()->UpdateDisplayPower();

            if (l_Caster->GetPower(POWER_RUNIC_POWER) == 0)
        ...
        void HandleScript(SpellEffIndex /*effIndex*//*)
        {
            if (Player* _player = GetCaster()->ToPlayer())
            {
                if (Unit* target = GetHitUnit())
                {
                    ...
                    _player->GetPlayerListInGrid(tempList, 20.f);
    ...
    class spell_dk_blood_mirror_AuraScript : public AuraScript
    {
        ...
        void HandleApply(AuraEffect const* /*aurEff*//*, AuraEffectHandleModes /*mode*//*)
        {
            ...
            if (caster->HasAura(SPELL_DK_BLOOD_MIRROR))
                caster->GetAura(SPELL_DK_BLOOD_MIRROR, target->GetGUID());
        }
        ...
        void HandleAbsorb(AuraEffect* aurEff, DamageInfo& dmgInfo, uint32& absorbAmount)
        {
            ...
            std::list<Unit*> targetList = caster->getAttackers().begin();

            if (!caster || targetList.empty())
                return;
    ...
    class spell_dk_bonestorm_AuraScript : public AuraScript
    {
        ...
        void HandleApply(AuraEffect const* /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            ...
            if (Unit* caster = GetCaster())
            {
                ...
                if (Player* player = caster->ToPlayer())
                    if (player->GetPower(POWER_RUNIC_POWER))
                        player->UpdateDisplayPower();
            }
        }
