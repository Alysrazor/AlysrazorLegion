...
class spell_dh_spirit_bomb : public SpellScript
{
    ...
    SpellCastResult CheckCast()
    {
        ...
        caster->GetAreaTriggers(SPELL_DH_SHATTERED_SOULS_AT_NORMAL);
        caster->GetAreaTriggers(SPELL_DH_SHATTERED_SOULS_AT_DEMON);
        caster->GetAreaTriggers(SPELL_DH_SHATTERED_SOULS_LESSER);
...
public:
    ...
    class spell_dh_fel_devastation_SpellScript : public SpellScript
    {
        ...
        void Register() override
        {
            OnHit += SpellHitFn(spell_dh_fel_devastation_SpellScript::Morph);
        }
    };
...
public:
    ...
    class spell_dh_fracture_SpellScript : public SpellScript
    {
        ...
        void HandleDummy(SpellEffIndex /*effIndex*/)
        {
            if (Unit* caster = GetCaster())
            {
                caster->CastCustomSpell(SPELL_DH_SHATTERED_SOULS_MISSILE, SpellValueMod(SPELL_EFFECT_TRIGGER_MISSILE_SPELL_WITH_VALUE + EFFECT_1), SPELL_DH_SHATTERED_SOULS_LESSER, caster, TRIGGERED_FULL_MASK);
                caster->CastCustomSpell(SPELL_DH_SHATTERED_SOULS_MISSILE, SpellValueMod(SPELL_EFFECT_TRIGGER_MISSILE_SPELL_WITH_VALUE + EFFECT_1), SPELL_DH_SHATTERED_SOULS_LESSER, caster, TRIGGERED_FULL_MASK);
            }
        }
...
public:
    ...
    class spell_dh_fiery_brand_dot_AuraScript : public AuraScript
    {
        ...
        void HandlePeriodic(AuraEffect const* aurEff)
        {
            ...
            if (Aura* burningAlive = caster->GetAura(SPELL_DH_BURNING_ALIVE))
            {
                if (owner == ObjectAccessor::GetUnit(*caster, owner->GetGUID()))
                {
    ...
    class spell_dh_shattered_souls_AuraScript : public AuraScript
    {
        ...
        void HandleProc(AuraEffect const* /*aurEff*/, ProcEventInfo& eventInfo)
        {
            ...
            if (roll_chance_i(GetSpellInfo()->GetEffect(EFFECT_0)->BasePoints))
            {
                ...
                caster->CastCustomSpell(SPELL_DH_SHATTERED_SOULS_MISSILE, SpellValueMod(SPELL_EFFECT_TRIGGER_MISSILE_SPELL_WITH_VALUE + EFFECT_1), triggerSpellId, caster);
            }
        }
...
public:
    ...
    private:
        ...
        bool Load() override
        {
            _charges = GetCaster()->GetSpellHistory()->GetChargeRecoveryTime(sSpellMgr->GetSpellInfo(SPELL_DH_FEL_BARRAGE)->ChargeCategoryId);
            return true;
        }
